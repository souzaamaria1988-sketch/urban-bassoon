...
public record RerollPayload(int villagerId) implements CustomPayload {
    public static final CustomPayload.Id<RerollPayload> ID = 
        new CustomPayload.Id<>(Identifier.of(ExampleMod.MOD_ID, "reroll"));
    
    public static final PacketCodec<PacketByteBuf, RerollPayload> CODEC = 
        PacketCodec.tuple(
            PacketCodec.INTEGER, RerollPayload::villagerId,
            RerollPayload::new
        );
